...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--skip_vis",
        action="store_true",
        help="Skip matplotlib intermediate visualization (less CPU, faster)",
    )
    parser.add_argument(
        "--points_per_side",
        type=int,
        default=32,
        help="SAM1 grid density (smaller=faster, fewer masks)",
    )
    ...
    # Official size from Meta CDN; truncated downloads cause zip archive errors on torch.load
    sam_ckpt_expected_bytes = 2_564_550_879
    if not os.path.isfile(sam_ckpt_path):
        raise FileNotFoundError(
            f"SAM1 checkpoint not found: {sam_ckpt_path}\n"
            "Download: cd checkpoints/sam1 && bash download.sh"
        )
    sam_ckpt_size = os.path.getsize(sam_ckpt_path)
    if sam_ckpt_size != sam_ckpt_expected_bytes:
        raise RuntimeError(
            f"SAM1 checkpoint size mismatch: {sam_ckpt_path}\n"
            f"  got {sam_ckpt_size} bytes ({sam_ckpt_size / 2**30:.2f} GiB)\n"
            f"  expected {sam_ckpt_expected_bytes} bytes ({sam_ckpt_expected_bytes / 2**30:.2f} GiB)\n"
            "Delete the file and re-download:\n"
            "  cd checkpoints/sam1 && rm -f sam_vit_h_4b8939.pth && bash download.sh"
        )
