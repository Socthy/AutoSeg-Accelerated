...
def frame_sort_key(filename):
    """Sort frames by numeric index; supports pure digits or prefixes like anli2_frame_000184."""
...
def images_to_video(input_folder, output_video, fps=30, max_width=1920, max_height=1080):
    # Get all PNG images from the input folder and sort by frame index
...
if __name__ == '__main__':
    ...
    if not os.path.isdir(imagebasedir):
        raise FileNotFoundError(f"Image directory not found: {imagebasedir}")
    if not os.path.isdir(npybasedir):
        raise FileNotFoundError(
            f"Mask output not found: {npybasedir}\n"
            "Run auto-mask-fast.py successfully first (same --output_dir and --level)."
        )

    # Load image and npy file lists
    image_name_list = [
        p for p in os.listdir(imagebasedir)
        if os.path.splitext(p)[-1] in IMAGE_EXTENSIONS
    ]
    image_name_list.sort(key=frame_sort_key)
